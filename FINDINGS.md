# FINDINGS.md — полный аудит кода pico-retro-new-gen (v2, актуализирован на r155)
...
| T8 | emu.c:154-160 | throttle elapsed+udelay | «60 Гц» | ✓ | самосогласовано | [fixed r127+]; настройка F2 убрана r158 — всегда 16667 |
...
| P26 | Enter/ESC теряются в wait_release | [fixed r155] таймаут 500 мс + re-init пада |
...
| throttle эмуляторов | emu.c:150-160 | кадровая частота | 60 Гц = 16667 мкс (фикс, r158; F2-настройка убрана) |
...
### FPS эмуляторов (фиксировано, r158)
- `emu_throttle` ждёт до `emu_period_us = 16667` мкс → ровно 60 Гц.
  Настройка частоты из меню (F2, Settings) убрана в r158.
- Паузы входа в игры: armed/sticky выход, re-init пада (r155-r157).
- **Тайминг без «ускорялок»**: A2600 mainloop останавливается на границе кадра (r158).
...
## 9. Правки r154–r155 (стабильность ввода, джойстик)

| № | Что | Где | Статус |
|---|-----|-----|--------|
| R1 | Выход из эмулятора с геймпада: **Start+Mode ~0,9 с** (кэш скана пада 50 мс) | emu.c | r155 |
| R2 | **armed-предохранитель**: удержание выхода не стартует до первого «чистого» кадра — «доезд» не выкидывает новую игру | emu.c | r155 |
| R3 | **Sticky-удержание выхода**: отсчёт рвётся только при отсутствии нажатия >250 мс подряд (клавиатура шлёт пачками) | emu.c | r155 |
| R4 | `usb_pad_wait_release` лимит **500 мс** + авто-`sega_pad_init` при «залипшем» паде (было до 1000 с) | usb_kbd.c | r155 |
| R5 | **Re-init пада** при входе/выходе эмулятора (rom_browser, main для portfolio/msx) | rom_browser.c, main.c | r155 |
| R6 | Кэш скана пада **12 мс** (было 2 мс) + пауза 16 мс в меню/настройках — частота I2C ~60/с, окно RMW не гоняется | usb_kbd.c, menu.c, settings.c | r155 |
| R7 | Клавиатура и джой **равноправны** в меню (`g_kbd_saved` — отложенный сканкод не теряется); клавиатура больше не «перебивает» джой | usb_kbd.c | r155 |
| R8 | **Пауза = Start, а не Mode**: Lynx Start→Pause (Mode освобождён), SMS/GG Mode больше не дублирует паузу | lynx_host.cpp, gpgx | r155 |
| R9 | LED-роли поменяны: **живой = PL10 (CPU1, R_PIO)**, **SD = PA15** — мигалка убрана с PA_DAT, осн. источник гонки устранён | led.c, tft_drv/комментарии | r155 |
| R10 | Тест джоя: тактирование исправлено (мкс-пауза вместо `*24`), выход по Start «sticky» | settings.c | r155 |
| R11 | `g_kbd_saved` сбрасывается в wait-releases (отложенный Enter/ESC не «выстреливает» в меню после игры) | usb_kbd.c | r155 |
| R12 | Ложные комментарии приведены к коду (i2s не вызывается, «safety» в SNES нет) | gpgx, snes_host, i2s.c, usb_kbd.h | r155 |

**Открытым остаётся (архитектурно):**
- RMW-гонка PA_DAT между CPU0 (I2C пада, SD-LED) и CPU1 (тач PA21) **не устранена радикально** —
  смягчена кэшем/паузами/re-init. Полное решение: аппаратный TWI0 для пада или сериализация PA_DAT.
- SNES/GB: нет safety-таймаута внутри `S9xMainLoop`/`emulator_run_until` (ROM-фриз = вечный кадр).
- Меню: `usb_input_poll` отдаёт одно событие за вызов (одновременные фронты джоя теряются, но не съедаются клавиатурой).
| R13 | **clash-защита линий D0-D3** в 6-btn протоколе: старшие кнопки (X/Y/Z/Mode) выдаются только если линия не занята крестовиной в фазе 1 — устранён «фантом Mode→Select» при движении вправо (A2600 «вправо = выбор игры») | sega_pad.c | r157 |
| R14 | **A2600-ускорение**: MCUME `mainloop` покрывал ~3.4 кадра (sim/s≈195 при run/s≈57); теперь останавливается на границе кадра (`tv_draw_count != frame0`), на железе run/s=sim/s=60 | mcume/Cpu.c, system_atari_h3.cpp | r158 |
| R15 | **Настройка частоты кадра убрана из меню** (HDMI-пункт, TFT-пункт, F2 в главном меню, TFT_PERIOD 0x74) — эмуляторы всегда 60 Гц | settings.c, tft_drv.c, menu.c | r158 |
