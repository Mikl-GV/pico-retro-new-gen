...
// Полупериод SCL. 524 тика HS-таймера (~97 МГц) = 5.4 мкс → SCL ~92 кГц.
// При 24 МГц было бы 130; реально 96.8 МГц → 524 (r127).
static inline void t_half(void) {
    const uint32_t t0 = H3_HS_TIMER->CURNT_LO;
    while ((t0 - H3_HS_TIMER->CURNT_LO) < 524) {
...
// r127: HSTMR реально ~97 МГц, поэтому 20*100=2000 тиков ≈ 20.6 мкс —
// РОВНО эталонные паузы. Множитель 100 оставлен как был (теперь честный).
...
// Паузы фазирования: 20*100=2000 тиков ≈ 20.6 мкс при ~97 МГц (r127).
