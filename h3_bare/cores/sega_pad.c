...
// Полупериод SCL. 130 тиков HS-таймера (24 МГц) = 5.4 мкс → SCL ~92 кГц.
// (Комментарий оригинала про 100 МГц остался от lib-h3 — таймер реально на 24 МГц.)
...
// ВАЖНО: ниже «микросекунды» завязаны на HS-таймер (24 МГц) как на 100 МГц —
// т.е. *_US * 100 тиков дают ~4 МГц-масштаб. Реально 20*100=2000 тиков
// = 2000/24 МГц ≈ 83 мкс (а не 20). Множитель оставлен для сохранения
// текущего поведения; при желании честных 20 мкс нужно * 24.
#define TH_SETTLE_US 20   // пауза после записи SELECT (установление TH), тики = 20*100
#define TH_HOLD_US   20   // пауза после чтения (дать линиям устояться)
#define TH_IDLE_US   20   // между фазами прокрутки счётчика

// Паузы фазирования. Множитель 100 — от «100 МГц»-происхождения lib-h3,
// но таймер на 24 МГц: 20*100=2000 тиков ≈ 83 мкс (см. примечание выше).
