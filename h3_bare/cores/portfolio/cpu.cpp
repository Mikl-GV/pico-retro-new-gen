...
/* Portfolio memory map:
 *  0x00000-0x1EFFF  RAM
 *  0x1F000-0x1FFFF  VRAM (tail of the 128K image)
 *  0xB0000-0xBFFFF  mirrored VRAM window (repeats 4K every 0x1000)
 *  0xC0000-0xDFFFF  ROM cartridge (bank view, from 256K ROM image offset 0)
 *  0xE0000-0xFFFFF  BIOS (from 256K ROM image offset 0x20000)
 */
void write86(uint32_t addr32, uint8_t value) {
  if (addr32 < RAM_SIZE) {
    write_ram(addr32, value);
  }
  else if ((addr32 >= 0xB0000) && (addr32 < 0xC0000)) {
    /* mirrored VRAM window -> last 4K of the RAM image */
    VRAM_write((addr32 - 0xB0000UL) & 0xFFF, value);
  }
}
...
uint8_t read86(uint32_t addr32) {
  if (addr32 < RAM_SIZE) {
    ...
    return VRAM_read((addr32 - 0xB0000UL) & 0xFFF);
  }
  else if (addr32 >= 0xE0000) {
    return pofo_rom_read(addr32 - 0xE0000UL + 0x20000);
  }
  else if (addr32 >= 0xC0000) {
    return pofo_rom_read(addr32 - 0xC0000UL);
  }
  else return 0;
}
...
static uint64_t lastcountertimer = 0, counterticks = 10000;
...
/* Debug accessors for the system layer (ip/segregs are file-static). */
extern "C" uint16_t pofo_get_ip(void) { return ip; }
extern "C" uint16_t pofo_get_cs(void) { return segregs[regcs]; }
