    ...
    for (int row = 0; row < 8; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            ...
            if (px >= 0 && px < FB_W && py >= 0 && py < FB_H) {
                // Работаем с volatile uint32* (вертикальные полосы в фреймбуфере)
                // Каждый пиксель — 4 байта XRGB8888
                if (bits & (0x80 >> col))
                    fb[py * FB_W + px] = color;
                else
                    fb[py * FB_W + px] = 0;
...
// Вывести строку (x,y = пиксель первой буквы). Гарантирует пробелы между строками.
