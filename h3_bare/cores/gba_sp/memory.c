...
file_tag_type gamepak_file_large = 0;
            ...
            case RTC_COMMAND:
              ...
              if(rtc_bit_count < 0)
              {
                switch(rtc_command)
                {
                  ...
                  {
                    /* Bare-metal: без time.h — отдаём фиксированную дату. */
                    rtc_state = RTC_OUTPUT_DATA;
                    rtc_data_bytes = 7;
                    rtc_data[0] = encode_bcd(26);   /* 2026 год */
                    rtc_data[1] = encode_bcd(1);    /* январь */
                    rtc_data[2] = encode_bcd(1);    /* 1-е */
                    rtc_data[3] = encode_bcd(1);    /* понедельник */
                    rtc_data[4] = encode_bcd(12);   /* 12:00 */
                    rtc_data[5] = encode_bcd(0);
                    rtc_data[6] = encode_bcd(0);
                    break;
                  }
                  ...
                  case RTC_COMMAND_OUTPUT_TIME:
                  {
                    rtc_state = RTC_OUTPUT_DATA;
                    rtc_data_bytes = 3;
                    rtc_data[0] = encode_bcd(12);   /* 12:00:00 */
                    rtc_data[1] = encode_bcd(0);
                    rtc_data[2] = encode_bcd(0);
                    break;
                  }
                }
                rtc_bit_count = 0;
