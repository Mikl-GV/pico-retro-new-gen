...
UWORD Screen_atari[ATARI_WIDTH / 2]; // = NULL;
#define scrn_line 0 //(ATARI_WIDTH / 2)

#define DRAWLINE() \
    if ( (ANTIC_ypos > 8) && (ANTIC_ypos < 248)) { \
      a5_DrawLinePal16((unsigned char *)&Screen_atari[32/sizeof(Screen_atari[0])], 320, 240, ANTIC_ypos-8); \
    }
...
#define GOEOL CPU_GO(ANTIC_LINE_C); ANTIC_xpos -= ANTIC_LINE_C; ANTIC_screenline_cpu_clock += ANTIC_LINE_C; UPDATE_DMACTL; DRAWLINE();ANTIC_ypos++; UPDATE_GTIA_BUG
		...
		if (ANTIC_player_gra_enabled) {
			...
			if (singleline) {
				if (ANTIC_xe_ptr != NULL && pmbase_s < 0x8000 && pmbase_s >= 0x4000)
					...
					base = memory + pmbase_s + ANTIC_ypos;
				if (ANTIC_ypos & 1) {
				...
				if (ANTIC_xe_ptr != NULL && pmbase_d < 0x8000 && pmbase_d >= 0x4000)
					...
					base = memory + pmbase_d + (ANTIC_ypos >> 1);
				if (ANTIC_ypos & 1) {
...
#define YPOS_BREAK_FLICKER do{}while(0)
	...
	} while (ANTIC_ypos < (ATARI_HEIGHT + 8));
	...
	if (ANTIC_pal_blending)
	{
		...
		do {
			...
			do {
				...
				WRITE_VIDEO_LONG(ptr, (READ_VIDEO_LONG(ptr - ATARI_WIDTH / 4) & mask) | pix);
			} while (--k);
