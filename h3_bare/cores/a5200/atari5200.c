...
// global variables
int tv_mode = TV_PAL;
extern unsigned char *memory;   /* points into the shared NES-buffer pool */
extern const unsigned char *at5_rom;   /* embedded cartridge ROM (flash) */
extern int at5_rom_size;
...
static void load_CART(char * cartname) 
{
  ...
  {
    case 32768: // 32k cart
      for (i = 0; i < 32768; i++) memory[0x4000 + i] = at5_rom[i];
    ...
    case 16384: // 16k cart
      ...
      for (i = 0; i < 16384; i++) memory[0x4000 + i] = memory[0x8000 + i] = at5_rom[i];
      ...
      a5_printf("Allocating p16kMaps");
      int num16kMappings = 0;      // pointer to 16k rom mappings and number of 16k rom mappings
      Map16k_t * p16kMaps = (Map16k_t *)emu_TmpMemory(); //a5_Malloc(sizeof(Map16k_t) * NUM_16K_ROM_MAPS);
      ...
      for(i=0; i<16384; i++) memory[0x6000 + i] = at5_rom[i];
  ...
  if (strncmp("pengo", memory + 0xBFEF, 8) == 0) {
    a5_printf("Pengo detected! Switching controller to Pengo mode.");
    pot_max_left = 70;
    pot_max_right = 170;
  }
  ...
  if (strncmp("centipede", memory + 0xBFEF, 8) == 0) {
    a5_printf("centipede detected! Switching controller to centipede mode.");
...
void at5_Init(void)
{
  ...
  {
    a5_PaletteEntry(R32(colourtable[i]), G32(colourtable[i]), B32(colourtable[i]), i);
  }
  ...
  /* memory is assigned by system_a5200.cpp from the shared pool before
     at5_Init() runs. */
...
extern int ik;     /* joypad key, set by system_a5200.cpp each frame */
static int ihk;    /* I2C keyboard key */
static int iusbhk; /* USB keyboard key */

/* emu_KeyboardOnDown/Up are provided by the shared atari2600_mcume layer. */

void at5_Input(int click) {
  ik  = a5_GetPad();
  ihk = a5_ReadI2CKeyboard();   
}
