...
// freestanding C++ не объявляет malloc/free/realloc/calloc — объявляем сами
extern "C" void* malloc(unsigned long size);
extern "C" void free(void* ptr);
extern "C" void* realloc(void* ptr, unsigned long size);
extern "C" void* calloc(unsigned long count, unsigned long size);

// Без <math.h> — freestanding C++ ломается на newlib requires_hosted.
// Нужна только floor(), реализуем сами (ближайший меньший double).
static inline double blip_floor(double x) {
    if (x != x) return x; // NaN
    if ((double)(long long)x == x) return x;
    return (x > 0) ? (double)(long long)x : (double)(long long)(x - 1.0);
}
#define floor blip_floor
