...
// koyote_bin is defined in ngp_host.cpp (from koyote_bin.h)
#define KOYOTE_BIN_SIZE 12288
extern unsigned char koyote_bin[];
...
// mainram, mainrom, cpurom, cpuram, realBIOSloaded are defined in ngp_host.cpp
extern unsigned char mainram[(64+32+128)*1024];
extern unsigned char mainrom[4*1024*1024];
extern unsigned char cpurom[256*1024];
...
{
	// On bare metal there is no NPBIOS.BIN file; the BIOS is loaded from
	// the internal koyote_bin by ngp_host.cpp before mainemuinit() runs.
	return 0;
}
...
{
	// stub - no file I/O on bare metal
}
