...
// Если кольцо пусто, а в аппаратном FIFO меньше 16 пар — заливаем
// тишину (0), чтобы I2S не гнал мусор при underrun'е («шум в обход
// громкости», особенно на ярких кадрах).
void i2s_flush(void) {
    if (!g_i2s_ready) return;
    ...
    // Кольцо пусто, а в аппаратном FIFO меньше 16 пар — подливаем тишину,
    // чтобы избежать underrun и мусора на DOUT («шум в обход громкости»).
    // I2S сам вычитает с правильным темпом — тишина уходит в воздух.
    while ((I2S_FIFO_STA & I2S_FIFO_STA_TXCNT_MASK) < (48u << 8)) {
        I2S_FIFO_TX = 0;
        I2S_FIFO_TX = 0;
    }
}
