...
int printf(const char* fmt, ...) {
    ...
    for (; *fmt; fmt++) {
        ...
        case 'd': case 'i': {
            ...
            if (v < 0) { uart_putc('-'); v = -v; }
            print_uint((unsigned long)v, 10, 0); break; }
        case 'u': print_uint((unsigned long)va_arg(ap, unsigned), 10, 0); break;
        case 'x': print_uint((unsigned long)va_arg(ap, unsigned), 16, 0); break;
        case 'X': print_uint((unsigned long)va_arg(ap, unsigned), 16, 1); break;
        case 'p': uart_puts("0x"); print_uint((unsigned long)va_arg(ap, void*), 16, 0); break;
        case '%': uart_putc('%'); break;
        default: uart_putc('%'); if (*fmt) uart_putc(*fmt); break;
        }
